...
func NewRouter(cfg config.Config, bot *tgbotapi.BotAPI, ads *service.AdsService, users *service.UsersService, store *session.Store, resolver *location.Resolver) *Router {
...
func (r *Router) handleMessage(ctx context.Context, upd tgbotapi.Update) {
	...
	u, err := r.users.GetUserAndEnsureRole(ctx, m.From)
	if err != nil {
		log.Printf("ensure user error: %v", err)
		return
	}
...
func isGlobalMenuAction(text string) bool {
	...
	case
		templates.BtnRoleProvider,
		templates.BtnRoleClient,
		templates.BtnRoleSwitch,
		templates.BtnCreateAd,
...
func (r *Router) handleCallback(ctx context.Context, upd tgbotapi.Update) {
	...
	msg := q.Message
	u, err := r.users.GetUserAndEnsureRole(ctx, q.From)
	if err != nil {
		log.Printf("ensure user error callback: %v", err)
		return
	}
	if u.Role == nil || *u.Role == "" {
		out := tgbotapi.NewMessage(msg.Chat.ID, templates.RoleSelectText())
		out.ReplyMarkup = templates.RoleSelectionKeyboard()
		_, _ = r.bot.Send(out)
		return
	}
	...
	if *u.Role == models.RoleClient &&
		(strings.HasPrefix(data, "confirm:") || strings.HasPrefix(data, "cancel:") || strings.HasPrefix(data, "my:taxi:")) {
		out := tgbotapi.NewMessage(msg.Chat.ID, "Bu amal faqat haydovchi/usta uchun.")
		out.ReplyMarkup = templates.ClientMenuKeyboard()
		_, _ = r.bot.Send(out)
		return
	}
